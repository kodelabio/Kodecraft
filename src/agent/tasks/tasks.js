...
export class Task {
  constructor(agent, task_data, taskStartTime = null) {
    ...
    if (task_data) {
      ...
      console.log("[TASK]: Task start time set to", this.taskStartTime);
      ...
      if (this.task_type === "construction") {
        ...
        this.validator = new CookingCraftingTaskValidator(
          this.data,
          this.agent,
        );
      } else {
  ...
  async teleportBots() {
    ...
    if (this.data.type !== "construction") {
      ...
      bot.chat(
        `/tp ${this.name} ${Math.floor(pos.x + xOffset)} ${pos.y + 3} ${Math.floor(pos.z + zOffset)}`,
      );
      await new Promise((resolve) => setTimeout(resolve, 200));
    }
    ...
    if (this.data.type === "construction") {
      ...
      if (this.blueprint) {
        ...
        bot.chat(
          `/tp @a ${first_coord[0]} ${first_coord[1]} ${first_coord[2]}`,
        );
        if (this.agent.agent_id === 0 && this.data.human_count > 0) {
          for (let i = 0; i < this.data.human_count; i++) {
            const username = this.data.usernames[i];
            await bot.chat(
              `/tp ${username} ${nearbyPosition.x} ${nearbyPosition.y} ${nearbyPosition.z}`,
            );
          }
        }
